/*To the already defined EMPLOYEE1 class, add two subclasses. FullTimeEmp and PartTimeEmp. 
Add the following data Members and Member Functions to the PartTimeEmp class. 
  hoursWorked(int): Represents the number of hours worked by the part-time employee. 
  hourly-rate(double, static and final): Represents the hourly rate at which the part-time employee is paid. 
  The calculateSalary(): overrides the calculateSalary() method of the base class to calculate the salary of the part-time employee 
                         based on the hours worked and hourly rate. 
  The displayEmployeeDetails(): Overrides the displayEmplyeeDetails() methods of the base class to display the part-time employee's details, 
